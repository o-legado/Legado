...
async function carregarDados() {
    const resposta = await fetch("../../../assets/data/liga/season1/ligaEquipes.json")
    equipes = await resposta.json()


    const resposta2 = await fetch("../../../assets/data/liga/season1/ligaJogadores.json")
    ...
    start(equipes) {
        ...
        for (let mesEquipes in equipes) {
                ...
                for (let indice in mesSelecionadoJson) {
                    const data = mesSelecionadoJson.data;

                    
                    
                        const treinoEquipe = mesSelecionadoJson.equipes;
                        
                        this.pegarDadosDasEquipes(treinoEquipe, data);
                        break;
